...
> [!IMPORTANT]
> When using C3 to create this project, select "no" when it asks if you want to deploy. You need to follow this project's [setup steps](https://github.com/cloudflare/templates/tree/main/blue-snow-97ac#setup-steps) before deploying.
...
```bash
npm create cloudflare@latest -- --template=cloudflare/templates/blue-snow-97ac
```

A live public deployment of this template is available at [https://blue-snow-97ac.templates.workers.dev](https://blue-snow-97ac.templates.workers.dev)
...
1. Install the project dependencies with a package manager of your choice:
   ...
   npm install
   ```
2. Create a [D1 database](https://developers.cloudflare.com/d1/get-started/) with the name "blue-snow-97ac-d1":
   ```bash
   npx wrangler d1 create blue-snow-97ac-d1
   ```
   ...and update the `database_id` field in `wrangler.json` with the new database ID.
3. Run the following db migration to initialize the database (notice the `migrations` directory in this project):
   ```bash
   npx wrangler d1 migrations apply --remote blue-snow-97ac-d1
   ```
